...
router.get("/device/:device_id", authenticateToken, async (req, res) => {
  try {
    const patient = await Patient.findOne({ device_id: req.params.device_id });
    if (!patient) return res.status(404).json({ message: "Patient not found" });
    res.status(200).json(patient);
  } catch (error) {
    console.error("Error fetching patient by device ID:", error);
    res.status(500).json({ message: "Internal Server Error" });
  }
});
